---
title: WSO2 Identity Platform Security Bulletin – H2 2025
...
| ASG-2025-012 | DoS vulnerability in SMS OTP flow in WSO2 Identity Platform | Medium | The process for sending an SMS runs in the same thread that serves the client request. Since there are a limited number of threads, configuring a slow or delayed endpoint for the SMS provider and continuously sending authentication requests can block those threads, potentially causing degraded performance or server unresponsiveness. |
...
| ASG-2025-014 | DoS via adaptive script in WSO2 Identity Platform | High | A vulnerable adaptive script could cause excessive memory consumption, potentially resulting in pod restarts and temporary service disruption. |
